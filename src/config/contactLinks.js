const WHATSAPP_NUMBER = '2349160264415';

export const PARTNER_WHATSAPP_MESSAGE =
    "We're building the future of campus connections. Interested in supporting, investing, or collaborating with The College Date? I'd like to connect.";

export const SUPPORT_WHATSAPP_MESSAGE = 'Hi, I need help with College Date.';

export const partnerWhatsAppUrl = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(PARTNER_WHATSAPP_MESSAGE)}`;

export const supportWhatsAppUrl = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(SUPPORT_WHATSAPP_MESSAGE)}`;
